...
export const Breadcrumb: React.FC<BreadcrumbProps> = ({ items, className = '' }) => {
  return (
    <nav aria-label="Breadcrumb" className={`flex items-center gap-2 text-body-sm text-neutral-600 ${className}`}>
      <a href="/" className="flex items-center hover:text-primary-navy transition-colors">
      ...
      {items.map((item, index) => (
        <React.Fragment key={index}>
          ...
          {item.href ? (
            <a href={item.href} className="hover:text-primary-navy font-medium transition-colors">
              {item.label}
            </a>
          ) : (
