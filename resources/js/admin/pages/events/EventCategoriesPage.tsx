...
const EventCategoriesPage: React.FC = () => {
  ...
  const [modal, setModal] = useState<{
    open: boolean;
    mode: 'create' | 'edit';
    category: Partial<EventCategory> | null;
  ...
  const [deleteModal, setDeleteModal] = useState<{ open: boolean; category: EventCategory | null }>({ open: false, category: null });
  ...
  const fetchCategories = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await eventService.getAdminEventCategories();
      setCategories(data.sort((a, b) => a.display_order - b.display_order));
    } catch (err: any) {
      setError(err?.response?.data?.message || 'Failed to load event categories');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modal.mode === 'edit' && modal.category?.id) {
        await eventService.updateEventCategory(modal.category.id, formData);
      } else {
        await eventService.createEventCategory(formData);
      }
      setModal({ open: false, mode: 'create', category: null });
      fetchCategories();
    } catch (err: any) {
      alert(err?.response?.data?.message || 'Failed to save event category');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await eventService.deleteEventCategory(deleteModal.category.id);
      setDeleteModal({ open: false, category: null });
      fetchCategories();
    } catch (err: any) {
      alert(err?.response?.data?.message || 'Failed to delete event category');
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in pb-12">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-h3 text-neutral-900 font-bold">Event Categories</h1>
            <p className="text-body-xs text-neutral-500">
              Organize events by type (e.g. Sunday Service, Youth, Conference, Outreach).
            </p>
          </div>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-neutral-200 overflow-hidden shadow-xs">
        {loading ? (
          ...
          <div className="px-5 py-12 text-center">
            ...
            <p className="text-body-sm text-neutral-500 mb-4">No event categories created yet</p>
            <button
              onClick={openCreateModal}
          ...
          <form onSubmit={handleSubmit} className="relative bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 space-y-4 animate-scale-in">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Sunday Worship, Youth Group"
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Short description of events in this category..."
                className="w-full px-3.5 py-2 border border-neutral-200 rounded-lg text-body-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20"
              />
            </div>
