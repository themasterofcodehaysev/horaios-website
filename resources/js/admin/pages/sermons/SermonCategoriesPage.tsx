...
const SermonCategoriesPage: React.FC = () => {
  ...
  const [modal, setModal] = useState<{
    open: boolean;
    mode: 'create' | 'edit';
    category: Partial<SermonCategory> | null;
  ...
  const [deleteModal, setDeleteModal] = useState<{ open: boolean; category: SermonCategory | null }>({ open: false, category: null });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modal.mode === 'edit' && modal.category?.id) {
        await sermonService.updateCategory(modal.category.id, formData);
      } else {
        await sermonService.createCategory(formData);
      }
      setModal({ open: false, mode: 'create', category: null });
      fetchCategories();
    } catch (err: any) {
      alert(err?.response?.data?.message || 'Failed to save sermon category');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await sermonService.deleteCategory(deleteModal.category.id);
      setDeleteModal({ open: false, category: null });
      fetchCategories();
    } catch (err: any) {
      alert(err?.response?.data?.message || 'Failed to delete sermon category');
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in pb-12">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-h3 text-neutral-900 font-bold">Sermon Categories</h1>
            <p className="text-body-xs text-neutral-500">
              Organize sermons by service type (e.g. Sunday Worship, Bible Study, Youth Service).
            </p>
          </div>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-neutral-200 overflow-hidden shadow-xs">
        {loading ? (
          ...
          <div className="px-5 py-12 text-center">
            ...
            <p className="text-body-sm text-neutral-500 mb-4">No sermon categories created yet</p>
            <button
              onClick={openCreateModal}
          ...
          <form onSubmit={handleSubmit} className="relative bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 space-y-4 animate-scale-in">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Sunday Worship, Youth Service"
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Short description of sermons in this category..."
                className="w-full px-3.5 py-2 border border-neutral-200 rounded-lg text-body-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20"
              />
            </div>
