...
import {
  ArrowLeft, Plus, BookOpen, Edit2, Trash2, CheckCircle,
  XCircle, Loader2, AlertTriangle, X, Hash, Image as ImageIcon,
} from 'lucide-react';
...
const SermonSeriesPage: React.FC = () => {
  ...
  const [modal, setModal] = useState<{
    open: boolean;
    mode: 'create' | 'edit';
    series: Partial<SermonSeries> | null;
  }>({ open: false, mode: 'create', series: null });
  ...
  const [deleteModal, setDeleteModal] = useState<{ open: boolean; series: SermonSeries | null }>({ open: false, series: null });
  ...
  const fetchSeries = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await sermonService.getAdminSeries();
      setSeriesList(data);
    } catch (err: any) {
      setError(err?.response?.data?.message || 'Failed to load sermon series');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openEditModal = (se: SermonSeries) => {
    ...
    setModal({ open: true, mode: 'edit', series: se });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      if (modal.mode === 'edit' && modal.series?.id) {
        await sermonService.updateSeries(modal.series.id, formData);
      } else {
        await sermonService.createSeries(formData);
      }
      setModal({ open: false, mode: 'create', series: null });
      fetchSeries();
    } catch (err: any) {
      alert(err?.response?.data?.message || 'Failed to save sermon series');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteModal.series) return;
    setDeleting(true);
    try {
      await sermonService.deleteSeries(deleteModal.series.id);
      setDeleteModal({ open: false, series: null });
      fetchSeries();
    } catch (err: any) {
      alert(err?.response?.data?.message || 'Failed to delete sermon series');
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in pb-12">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-h3 text-neutral-900 font-bold">Sermon Series</h1>
            <p className="text-body-xs text-neutral-500">
              Group sermons by teaching themes (e.g. Romans, Faith Foundations, Easter).
            </p>
          </div>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-neutral-200 overflow-hidden shadow-xs">
        {loading ? (
          ...
          <div className="px-5 py-12 text-center">
            ...
            <button onClick={fetchSeries} className="text-body-sm text-primary-red font-medium hover:underline">Retry</button>
          </div>
        ) : seriesList.length === 0 ? (
          <div className="px-5 py-12 text-center">
            <BookOpen className="w-8 h-8 text-neutral-300 mx-auto mb-3" />
            <p className="text-body-sm text-neutral-500 mb-4">No sermon series created yet</p>
            <button
              onClick={openCreateModal}
            ...
            >
              <Plus className="w-4 h-4" /> Add First Series
            </button>
          </div>
        ) : (
          <div className="divide-y divide-neutral-100">
            <div className="hidden sm:grid grid-cols-[50px_60px_1fr_120px_100px_90px] gap-3 px-6 py-3 bg-neutral-50 border-b border-neutral-200 text-body-xs text-neutral-500 font-medium uppercase tracking-wide">
              <span>Order</span>
              <span>Image</span>
              <span>Series Name & Description</span>
            ...
            {seriesList.map((se) => (
              <div key={se.id} className="grid grid-cols-1 sm:grid-cols-[50px_60px_1fr_120px_100px_90px] gap-3 items-center px-6 py-4 hover:bg-neutral-50/60 transition-colors">
                ...
                <div className="min-w-0">
                  <h3 className="text-body-sm font-bold text-neutral-900">{se.name}</h3>
                  {se.description && <p className="text-body-xs text-neutral-500 truncate">{se.description}</p>}
                </div>

                <div>
                  <span className="inline-block text-body-xs px-2.5 py-0.5 rounded-full bg-accent-blue/10 text-accent-blue font-semibold">
                    {se.sermons_count ?? 0} Sermons
                  </span>
                </div>

                <div>
                  <span className={`inline-flex items-center gap-1 text-body-xs px-2.5 py-0.5 rounded-full font-medium ${
                    se.status === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-neutral-100 text-neutral-500'
                  }`}>
                    {se.status === 'active' ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                    {se.status}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-end gap-1">
                  ...
                  <button
                    onClick={() => setDeleteModal({ open: true, series: se })}
          ...
          <form onSubmit={handleSubmit} className="relative bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 space-y-4 animate-scale-in">
            <div className="flex items-center justify-between border-b border-neutral-100 pb-3">
              <h3 className="text-h5 text-neutral-900 font-bold">
                {modal.mode === 'create' ? 'Create Sermon Series' : 'Edit Sermon Series'}
              </h3>
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Romans: The Power of the Gospel"
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Overview of this sermon series..."
                className="w-full px-3.5 py-2 border border-neutral-200 rounded-lg text-body-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20"
              />
            </div>
      ...
      {/* Delete Modal */}
      {deleteModal.open && deleteModal.series && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setDeleteModal({ open: false, series: null })} />
          <div className="relative bg-white rounded-2xl shadow-xl max-w-md w-full p-6 animate-scale-in">
            <div className="w-12 h-12 rounded-full bg-red-100 flex items-center justify-center mx-auto mb-4">
              <AlertTriangle className="w-6 h-6 text-red-600" />
            </div>
            <h3 className="text-h5 text-neutral-900 text-center font-semibold mb-2">Delete Sermon Series</h3>
            <p className="text-body-sm text-neutral-500 text-center mb-6">
              Are you sure you want to delete <strong>"{deleteModal.series.name}"</strong>?
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setDeleteModal({ open: false, series: null })}
